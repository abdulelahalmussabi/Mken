...
console.log('=== [SMOKE TEST] Purchases & Vendors Module (mken_vendors & mken_purchase_invoices) ===');
...
async function runSmokeTests() {
  console.log('\n2. Testing GET /api/purchases (Tenant Scope Isolation):');
  ...
  try {
    ...
    const res1 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/purchases?tenant_slug=almahrusa',
      method: 'GET',
      headers: {
    ...
    if (res1.data) {
      ...
      console.log('   - Vendors count:', Array.isArray(res1.data.vendors) ? res1.data.vendors.length : 0);
      console.log('   - Purchase invoices count:', Array.isArray(res1.data.invoices) ? res1.data.invoices.length : 0);
    }
    ...
    const res2 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/purchases?tenant_slug=demo',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'demo',
      },
    ...
    console.log('\n=== Purchases & Vendors Smoke Test Completed Successfully! ===');
    process.exit(0);
  } catch (err) {
