...
console.log('=== [SMOKE TEST] Settings & Config Data Module (mken_saas_clients.config_data) ===');
...
async function runSmokeTests() {
  console.log('\n2. Testing GET /api/settings (Tenant Scope & Secret Masking):');
  ...
  try {
    ...
    const res1 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/settings?tenant_slug=almahrusa',
      method: 'GET',
      headers: {
    ...
    if (res1.data) {
      ...
      console.log('   - Facility Name present:', !!res1.data.settings?.facility_name);
    }
    ...
    const res2 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/settings?tenant_slug=demo',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'demo',
      },
    ...
    console.log('\n=== Settings Smoke Test Completed Successfully! ===');
    process.exit(0);
  } catch (err) {
