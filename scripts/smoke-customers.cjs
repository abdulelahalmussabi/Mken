...
console.log('=== [SMOKE TEST] Customers Module (mken_customers) ===');
...
async function runSmokeTests() {
  console.log('\n2. Testing GET /api/customers (Tenant Scope Isolation):');
  ...
  try {
    ...
    const res1 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/customers?tenant_slug=almahrusa',
      method: 'GET',
      headers: {
    ...
    if (res1.data) {
      ...
      console.log('   - Customers count:', Array.isArray(res1.data.customers) ? res1.data.customers.length : 0);
    }
    ...
    const res2 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/customers?tenant_slug=demo',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'demo',
      },
    ...
    console.log('\n=== Customers Smoke Test Completed Successfully! ===');
    process.exit(0);
  } catch (err) {
