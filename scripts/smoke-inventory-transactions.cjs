...
console.log('=== [SMOKE TEST] Inventory Transactions Audit Module (mken_inventory_transactions) ===');
...
async function runSmokeTests() {
  console.log('\n2. Testing GET /api/inventory-transactions (Tenant Scope Isolation & Filtering):');
  ...
  try {
    ...
    const res1 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/inventory-transactions?tenant_slug=almahrusa',
      method: 'GET',
      headers: {
    ...
    if (res1.data) {
      ...
      console.log('   - Transactions count:', Array.isArray(res1.data.transactions) ? res1.data.transactions.length : 0);
    }

    // GET with Filter (type=stock-in)
    const resFilter = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/inventory-transactions?tenant_slug=almahrusa&type=stock-in',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'almahrusa',
      },
    ...
    console.log('   - GET with stock-in filter status:', resFilter.statusCode);

    // GET for Tenant 2 (demo)
    const res2 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/inventory-transactions?tenant_slug=demo',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'demo',
      },
    ...
    console.log('\n=== Inventory Transactions Smoke Test Completed Successfully! ===');
    process.exit(0);
  } catch (err) {
