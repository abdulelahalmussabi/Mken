...
console.log('=== [SMOKE TEST] Inventory Module (mken_inventory_items & transactions) ===');
...
async function runSmokeTests() {
  console.log('\n2. Testing GET /api/inventory (Tenant Scope Isolation):');
  ...
  try {
    ...
    const res1 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/inventory?tenant_slug=almahrusa',
      method: 'GET',
      headers: {
    ...
    if (res1.data) {
      ...
      console.log('   - Items count:', Array.isArray(res1.data.items) ? res1.data.items.length : 0);
    }
    ...
    const res2 = await makeRequest({
      hostname: 'localhost',
      port: PORT,
      path: '/api/inventory?tenant_slug=demo',
      method: 'GET',
      headers: {
        'x-tenant-slug': 'demo',
      },
    ...
    console.log('\n=== Inventory Smoke Test Completed Successfully! ===');
    process.exit(0);
  } catch (err) {
