...
// 4. Add env variables to Vercel using CLI with stdin piping to avoid shell quoting issues
console.log('Adding LICENSE_PRIVATE_KEY to Vercel (via stdin)...');
execSync('vercel env add LICENSE_PRIVATE_KEY production --yes', {
  input: envPrivateKey,
  stdio: ['pipe', 'inherit', 'inherit']
});

console.log('Adding LICENSE_PUBLIC_KEY to Vercel (via stdin)...');
execSync('vercel env add LICENSE_PUBLIC_KEY production --yes', {
  input: envPublicKey,
  stdio: ['pipe', 'inherit', 'inherit']
});

console.log('Adding LICENSE_ADMIN_TOKEN to Vercel (via stdin)...');
execSync('vercel env add LICENSE_ADMIN_TOKEN production --yes', {
  input: adminToken,
  stdio: ['pipe', 'inherit', 'inherit']
});
