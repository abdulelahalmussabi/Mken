...
function buildTenantContext(config) {
  ...
  lines.push('- رابط الحجز والدفع المباشر: https://' + domain + '/book');
  lines.push('- متجر المنتجات: https://' + domain + '/store');
...
function buildSystemPrompt(config, tenantContext, customerPhone, appointmentsInfo, conversationHistory, activityId) {
  ...
  let prompt = 'أنت سعد، المساعد الذكي في "' + brandName + '".';
  ...
  prompt += '- رابط الحجز: https://' + domain + '/book (لا تكرره في كل رسالة، فقط عند الحاجة).\n\n';
  ...
  prompt += '- لا تخترع أوقاتاً من عندك أبداً. إذا طلب العميل الحجز، أخبره أنك ستتحقق من الفراغ في النظام، أو استخدم أداة list_slots إذا كانت متاحة.\n';
  prompt += '- لتأكيد الحجز، استخدم أداة create_booking إذا جمعت كل البيانات (الاسم، الخدمة، التاريخ، الوقت).\n';
  prompt += '- إن لم تعرف إجابة، اعتذر بأدب ووجّه العميل للحجز.\n';
  prompt += '- لا تذكر أنك ذكاء اصطناعي — أنت سعد، المساعد الذكي في ' + brandName + '.\n';
  prompt += '- عند الترحيب قدّم نفسك: «أنا سعد مساعدك الذكي». ذكّر باسم ' + brandName + '، لا تستخدم أسماء أخرى.\n';
...
function getModelCandidates() {
  ...
  return override ? [override].concat(list.filter(function (m) { return m !== override; })) : list;
}
...
async function callGemini(systemPrompt, contents) {
    ...
    tools: [
      {
        functionDeclarations: [
          {
            name: "list_slots",
            description: "جلب الأوقات المتاحة للحجز في تاريخ معين.",
            parameters: {
              type: "object",
              properties: {
          ...
          {
            name: "create_booking",
            description: "إنشاء موعد جديد للعميل بعد التحقق من توفر الوقت.",
            parameters: {
              type: "object",
              properties: {
                ...
                serviceId: { type: "string", description: "معرف الخدمة (ID) من الكتالوج" },
                serviceName: { type: "string", description: "اسم الخدمة" },
                servicePrice: { type: "string", description: "سعر الخدمة كأرقام فقط (بدون عملة) إذا كانت مسعرة" }
              },
              required: ["name", "date", "time", "serviceId"]
            }
          }
        ]
      }
  ...
  for (const model of models) {
    ...
    try {
      ...
      return { type: 'text', text: extractReply(text) };
    } catch (e) {
...
async function generateAIReply(userMessage, config, tenantSlug, customerPhone, appointmentsInfo, conversationHistory, activityId) {
  ...
  try {
    ...
    if (reply && reply.type === 'tool_call') {
      return reply;
    }
    return sanitizeReply(reply ? reply.text : '');
  } catch (err) {
