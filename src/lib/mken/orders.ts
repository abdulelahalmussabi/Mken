...
/** Public tracking: order id + last 4 phone digits. Never returns the full phone. */
export async function fetchPublicOrderTrack(
  id: string,
  phone: string
): Promise<{ order?: Pick<Order, "id" | "tenantSlug" | "activityTitle" | "status" | "createdAt" | "items">; error?: string; notFound?: boolean }> {
  const db = getTenantDb();
  if (!db) return { error: "قاعدة البيانات غير مهيأة على الخادم" };

  const needle = digitsOnly(phone);
  if (needle.length < 4) return { error: "أدخل رقم الجوال كاملاً أو آخر 4 أرقام" };

  const { data, error } = await db.from("mken_orders").select("*").eq("id", id).maybeSingle();
  if (error) return { error: error.message };
  if (!data) return { notFound: true, error: "الطلب غير موجود" };

  const order = toOrder(data as OrderRow);
  const stored = digitsOnly(order.phone);
  if (!stored.endsWith(needle.slice(-4))) {
    return { notFound: true, error: "الطلب غير موجود" };
  }
