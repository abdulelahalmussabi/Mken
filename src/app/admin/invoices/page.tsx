...
export default function AdminInvoicesPage() {
  ...
  const [zatca, setZatca] = useState<ZatcaStatus | null>(null);
  const [zatcaLoading, setZatcaLoading] = useState(false);
  const [zatcaOnboardBusy, setZatcaOnboardBusy] = useState(false);
  const [zatcaLogs, setZatcaLogs] = useState("");
  ...
  const loadZatcaStatus = useCallback(async () => {
    if (!tenant) return;
    if (!isSuperAdmin && !saas.hasInvoices) return;
    setZatcaLoading(true);
    try {
      const res = await fetch(`/api/zatca${query}`);
      const data = await res.json();
      if (res.ok && data.success) {
        setZatca(data);
        if (data.vatNumber) {
  ...
  const runZatcaOnboard = async () => {
    if (!zatcaForm.vatNumber.trim() || !zatcaForm.businessName.trim() || !zatcaForm.otp.trim()) {
      showToast("يرجى ملء الرقم الضريبي واسم المنشأة ورمز OTP", "error");
      return;
    }
    setZatcaOnboardBusy(true);
    setZatcaLogs("[بدء الفحص والامتثال]\n");
    try {
      const res = await fetch(`/api/zatca${query}`, {
        method: "POST",
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        throw new Error(data.error || "حدث خطأ أثناء الاتصال بالهيئة");
      }
      if (Array.isArray(data.logs)) setZatcaLogs(data.logs.join("\n"));
      showToast("تم الربط والتحقق بنجاح مع ZATCA!", "success");
      await loadZatcaStatus();
    } catch (err) {
      const message = err instanceof Error ? err.message : "تعذّر الربط";
      setZatcaLogs((prev) => `${prev}\n[خطأ]: ${message}`);
      showToast(message, "error");
    } finally {
      setZatcaOnboardBusy(false);
    }
  };

  const reportToZatca = async (invoice: Invoice) => {
    setBusyId(invoice.id);
    try {
      const res = await fetch(`/api/zatca${query}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "report", invoice }),
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        showToast(data.error || "تعذّر الإبلاغ لهيئة الزكاة", "error");
        return;
      }
      setInvoices((prev) =>
        prev.map((i) =>
          i.id === invoice.id
            ? {
                ...
                zatcaQrCode: data.zatcaQrCode,
              }
            : i
        )
      );
      showToast(
        data.zatcaStatus === "REPORTED" ? "تم الإبلاغ بنجاح لهيئة الزكاة" : "تعذّر الإبلاغ",
        data.zatcaStatus === "REPORTED" ? "success" : "error"
      );
      load();
    } catch {
  ...
  return (
    <>
      ...
      <div className="space-y-6 text-right">
        ...
        {!tableMissing && (
          <div className="p-5 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-lg space-y-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <button
                type="button"
                onClick={loadZatcaStatus}
                disabled={zatcaLoading}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-xl transition-all disabled:opacity-50"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${zatcaLoading ? "animate-spin" : ""}`} />
                تحديث حالة ZATCA
              </button>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-bold">
                ...
                <span>الفوترة الإلكترونية (ZATCA)</span>
              </div>
            </div>

            {zatcaLoading ? (
              <p className="text-xs text-slate-400 font-bold">جاري تحميل حالة الربط...</p>
            ) : zatca?.configured ? (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  />
                  <input
                    value={zatcaForm.businessName}
                    onChange={(e) => setZatcaForm((p) => ({ ...p, businessName: e.target.value }))}
            ...
            {zatcaLogs && (
              <pre className="p-3 rounded-2xl bg-slate-950 border border-slate-800 text-[10px] text-slate-400 whitespace-pre-wrap text-right max-h-40 overflow-y-auto">
                {zatcaLogs}
              </pre>
            )}
          </div>
