...
const SERVICES_LIST: ServiceOption[] = [
  {
    id: "std-room",
    name: "غرفة قياسية (Standard Room)",
    duration: "حجز ليلة كاملة",
    price: "250 ر.س",
    desc: "سرير مزدوج أو ثنائي، شاشة ذكية، واي فاي فائق السرعة، وخدمة الغرف.",
  },
  {
    id: "deluxe-suite",
    ...
    duration: "حجز ليلة كاملة",
    price: "420 ر.س",
    desc: "غرفة نوم وصالة واسعة، إطلالة بانورامية، آلة اسبريسو وموقف خاص.",
  },
  {
    id: "family-suite",
    name: "جناح عائلي ملكي (Royal Family Suite)",
    duration: "حجز ليلة كاملة",
    price: "680 ر.س",
    desc: "غرفتا نوم، صالة، 2 حمام، مطبخ كامل وخدمة تنظيف يومية.",
  },
  {
    id: "vip-consult",
    name: "جلسة معاينة وحجز خاص VIP",
    duration: "30 دقيقة",
    price: "مجاناً",
    desc: "زيارة استكشافية للمجمع ومعاينة جميع الأجنحة المتاحة مع المشروبات.",
  },
];

function BookAppointmentContent() {
  const searchParams = useSearchParams();
  const tenantSlug = searchParams.get("tenant") || searchParams.get("store") || "almahrusa";

  const { activeOccasion, occasionDetails, openModal } = useOccasion();
  const { showToast } = useApp();

  const [selectedService, setSelectedService] = useState<ServiceOption>(SERVICES_LIST[0]);
  ...
  const tenantName = tenantSlug === "almahrusa" ? "مجموعة المحروسة" : `منشأة ${tenantSlug}`;
  ...
  const handleSubmitBooking = (e: React.FormEvent) => {
    e.preventDefault();
    if (!clientName.trim() || !clientPhone.trim() || !selectedDate) {
    ...
    setTimeout(() => {
      ...
      window.open(`https://wa.me/966551234567?text=${text}`, "_blank");
    }, 1000);
  };
    ...
    <div className="min-h-screen flex flex-col bg-theme-main text-slate-100 font-sans transition-colors duration-500 relative">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/85 backdrop-blur-xl border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
          <Link href={`/subscriber/${tenantSlug}`} className="flex items-center gap-3 group">
            <div
      ...
      >
        ...
        <span>عرض {occasionDetails.shortName}: <strong>{occasionDetails.discountText}</strong></span>
