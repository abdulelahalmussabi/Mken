...
انسخ `.env.example` إلى `.env.local` للتشغيل المحلي. الإنتاج يُضبط في Vercel فقط — لا ترفع `.env.local`.

المفاتيح العامة (`NEXT_PUBLIC_*`) تظهر في المتصفح. مفاتيح الخدمة والجلسات تبقى على الخادم.
